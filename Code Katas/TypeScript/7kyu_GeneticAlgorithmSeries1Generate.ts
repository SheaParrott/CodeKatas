...
// A genetic algorithm is based in groups of chromosomes, called populations. To start our population of chromosomes
...
export function generate(length: number): string {
    ...
    for (let i = 0; i < length; i++) returnValue+= `${Math.round(Math.random())}`;
    return returnValue;
};