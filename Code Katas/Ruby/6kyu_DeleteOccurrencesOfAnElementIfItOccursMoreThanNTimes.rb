...
# pictures of the places they've been, and now they want 
# to show Charlie their entire collection. However, Charlie 
# doesn't like this sessions, since the motive usually 
# repeats. He isn't fond of seeing the Eiffel tower 40 times. 
# He tells them that he will only sit during the session if 
# they show the same motive at most N times. Luckily, Alice 
# and Bob are able to encode the motive as a number. Can you 
# help them to remove numbers such that their list contains 
...
def delete_nth(order,max_e)
  answer = []
  order.each do |value|
    if answer.count(value) < max_e
       answer.concat(Array.new(1, value))
    end
  end
  answer
end
